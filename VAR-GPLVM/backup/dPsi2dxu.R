dPsi2dxu <- function(kpp,j,mod){
  ...
  for (i in 1:t){
    ...
    temp[kpp,] <- as.vector(-wj*(xukj-xu[,j])/2+wj*(2*mu[i,j]-xukj-xu[,j])/(2*(2*wj*Si+1)))
    temp[,kpp] <- as.vector(wj*(xu[,j]-xukj)/2+wj*(2*mu[i,j]-xukj-xu[,j])/(2*(2*wj*Si+1)))
    temp[kpp,kpp] <- as.numeric(wj*(2*mu[i,j]-2*xukj)/((2*wj*Si+1)))
